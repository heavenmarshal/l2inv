...
{
    sig2 <- py$coeffs2*py$d2
    ## sig2 <- py$coeffs2*py$d2+py$varres
    n <- ncol(sig2)
    p <- nrow(sig2)
    mu2 <- py$d2*(py$coeff-cht)^2
...
projeiinfo <- function(py,alpha,cht,valist)
{
    sig2 <- py$coeffs2*py$d2+py$varres
    n <- ncol(sig2)
    p <- nrow(sig2)
...
## returns saddle points for illustration
oeiinfowithsp <- function(py,alpha,cht,valist)
{
    yobs <- valist$yobs
    d2 <- py$d2
    L <- length(yobs)
    ...
    mumk <- z2+scalec-2*bzc+apply(scales2,2,sum)+py$varres*L
    mumk <- mumk - valist$barval
    ret <- .C("oeiinfowithsp_R", as.integer(n), as.integer(p),
              as.integer(L), as.double(py$varres),
              as.double(valist$barval),as.double(iomemu2),
              as.double(bound), as.double(amat),
              as.double(mub2star), as.double(mumk),
              info = double(n), saddlepoints=double(n),
              PACKAGE="l2inv")
    info <- ret$info-mumk
    rlist <- list(info=info,saddlepoints=ret$saddlepoints)
    return(rlist)
}
