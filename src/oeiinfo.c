	     ...
	     const double* mumk, double *info)
{
  ...
  int i, j, stat;
#ifdef __DEBUG__
  FILE *fptr = fopen("oeitrace.txt","w");
#endif
...
#ifdef __DEBUG__
    double fval = oeidkappaSeq(tval, (void*) &ppar);
    fprintf(fptr, "i=%d,\tstat=%d,\tt=%f,\tdk-barval=%f,\tk=%f,\tdk2=%f,\tdk3=%f\n", i, stat, tval, fval, kp, dk2, dk3);
#endif
  }
#ifdef __DEBUG__
  fclose(fptr);
